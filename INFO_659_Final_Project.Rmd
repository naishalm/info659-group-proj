...
# Ensure the prepare_xgb_data function is correctly defined
prepare_xgb_data <- function(data) {
  # One-hot encode categorical variables and prepare X and y
  data_encoded <- data %>%
    mutate(across(where(is.character), as.factor)) %>%
    mutate(across(where(is.factor), as.numeric))
  
  X <- as.matrix(data_encoded %>% select(-Outcome))
  y <- as.numeric(data$Outcome == "Transported to Hospital")
...
auc_result <- roc(test_xgb_data$y, test_pred)
...
# Feature importance using xgboost's inbuilt function
